...
`stream: true` flips the provider chain to streaming-capable upstreams only: **GPTProto / OpenRouter / APIMart / Packy** (for Claude) and **kie.ai** (for Claude on its Anthropic-native endpoint). Providers that proxy through Cloudflare-style edge buffering get filtered out — their streams die at ~30 s even on internally healthy responses.
