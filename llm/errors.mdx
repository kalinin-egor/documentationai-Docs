...
**6. Reference media rejected at submit.** The upstream fetched your `video_url` / `image_url` and refused it. Almost always the host, not the file: the fetch needs a direct link, a real media `Content-Type` (`video/mp4`, not `application/octet-stream`), and no auth. Google Drive / Dropbox share links fail this — see [Reference media: what format to send](/llm#reference-media-what-format-to-send).

**Fix:** re-host on S3/R2 or any static bucket with the right `Content-Type`. For *images* only, sidestep hosting entirely by sending a `data:` URI; video and audio references must be HTTPS URLs.
