...
<Update label="2026-05-14" description="Kling video — multi-provider auto-fallback, parametric −10% off Fal pricing, Avatar SKUs" tags={["feature", "video", "kling", "pricing"]}>

## Production Kling video — cheapest-of-two routing with transparent fallback

Kling video generation now ranks two upstreams per request (kie.ai +
PoYo) by their parametric source price for the exact `(slug,
...
The response now exposes `provider`, `provider_chain`, and a
`config` snapshot of the resolved generation parameters so callers
can see which upstream serviced the job:
...
## Kling retail is now anchored at Fal × 0.90

Every Kling SKU with a published Fal.ai listing is billed at exactly
**−10% off the Fal public price** for the requested configuration —
the same headline regardless of which upstream actually served the
job. A 10% margin floor against the chosen supplier guarantees we
never bill below source cost, so on a provider fallback the user
may see a 1-3% price bump but never a freebie. Catalog SKUs surface
...
Parametric examples (cheapest provider):

| SKU | Config | Retail | Margin |
|-----|--------|--------|--------|
| `kling-2-6` | 5s no-audio | $0.315 | 14.5% |
| `kling-2-6` | 5s with audio | $0.630 | 14.5% |
| `kling-3-0` | 1080p+audio 5s | $0.756 | 12% |
| `kling-3-0` | 4K 5s | $1.890 | 13% |
| `kling-2-1` | Pro 5s i2v | $0.405 | 170% |
| `kling-2-6-motion-control` | 1080p 5s v2v | $0.504 | 68% |
| `kling-avatar-pro` | 10s 1080p | $1.035 | 29% |
...
billing, kie.ai-exclusive — PoYo doesn't offer lip-sync today.
...
CrazyRouter's `model_not_found` / `insufficient_user_quota` patterns,
vendor maintenance envelopes). Previously, only the cheapest provider
was tried before falling straight to CrazyRouter as a safety net —
middle entries (Packy / GPTProto / APIMart / PoYo / kie.ai) were
...
## kie.ai routes Claude via Anthropic-native `/claude/v1/messages`

For Claude haiku/sonnet/opus 4-5/4-6, the kie.ai provider now serves
through the Anthropic-native endpoint with real SSE event streaming
(verified end-to-end on opus 4.6 / sonnet 4.6). Source rates are ~71 %
off Anthropic list — almost identical to Packy aws-q. The
redundancy benefit: if Packy throttles, kie.ai picks up at the same
price point instead of skipping to GPTProto's Bedrock account.
