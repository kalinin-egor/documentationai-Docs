...
For tools that expect the OpenAI protocol (Claude Code via LiteLLM, Cursor, Continue.dev), use the drop-in [OpenAI-compatible endpoint](/llm#openai-compatible-endpoint) at `https://api.gpuniq.com/v1/openai` instead.
...
Text-to-image and image-to-image through Nano Banana, Nano Banana Pro / 4K, and Grok 4 Image. See the [LLM & Image API](/llm) page for model slugs and pricing.
