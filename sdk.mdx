...
GPUniq gives you Claude (Opus 4.7, Sonnet 4.6, Haiku 4.5), GPT-5 family, Gemini 3, Grok 4, and 30+ open-source models under one API key. For tools that expect the OpenAI protocol (Claude Code via LiteLLM, Cursor, Continue.dev), use the drop-in [OpenAI-compatible endpoint](/llm#openai-compatible-endpoint) at `https://api.gpuniq.com/v1/openai` instead.

```python
# Simple chat
response = client.llm.chat("claude-haiku-4-5", "Explain transformers")
print(response)
