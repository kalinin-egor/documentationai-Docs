...
<Info>
  Image requests may take **up to ~85 seconds** end-to-end for higher-resolution
  variants (`nano-banana-2`, `nano-banana-pro`, `nano-banana-pro-4k`). The response
  is returned as a single JSON body once generation completes — no client-side
  polling is required. Set your HTTP client timeout to **at least 120 seconds**
  for these models.
</Info>
