...
<Callout kind="info">
  **Kling SKUs are billed at −10% off the Fal.ai public price**. The
  headline above is the cheapest default configuration (1080p / no
  audio / 5s / Pro tier). Audio, longer duration, 4K, and Master tier
  scale the price linearly off the underlying Fal rate × 0.9 — the
...
job_id = start["data"]["job_id"]
print(f"chain: {start['data']['provider_chain']}, est cost: ${start['data']['estimated_cost_usd']}")

# 2. Poll — Kling delivers in 30-90s; budget 5 minutes for the slowest variants
deadline = time.time() + 300
while time.time() < deadline:
    time.sleep(3)
...
parameter snapshot, and the upstream provider chain selected for this
request (cheapest first). The polling endpoint surfaces the current
upstream and falls back transparently if the provider in flight errors
out — your `job_id` and the user-facing price stay stable across
fallbacks.
