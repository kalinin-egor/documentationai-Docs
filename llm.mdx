...
| Seedance 2 | `seedance-2` | $0.33 / s | ByteDance 720p, **billed per second** (5s ≈ $1.65) |
...
| `seedance-2` | Seedance 2 | 720p | 5s | t2v, i2v | **Per second — $0.33 / s** |

<Callout kind="warning">
  **`seedance-2` is billed per second of generated video, not per
  clip.** A 5-second generation (the default) costs 5 × $0.33 =
  **$1.65**; a 10-second generation costs **$3.30**. The two `-pro`
  SKUs, by contrast, are a flat per-video price regardless of duration.
  Always read the `cost_usd` field of the completion response for the
  exact amount charged — and the `estimated_cost_usd` on the kickoff
  response, which already accounts for the duration you requested.
</Callout>
...
Seedance shares the standard [video job API](#video-generation) —
`duration` and `image_url` are the two fields that matter for it;
`aspect_ratio` is honoured where the upstream supports it. Fields
specific to other families (`audio`, `mode`, `resolution` tiers,
`video_url` motion-control) are ignored by Seedance.
