...
If the 2K/4K upstream rejects a GPT Image 2 request's *content*
(prompt/reference images), GPUniq retries it once as the base
`gpt-image-2` at the base price ($0.0464) — you get a ~1.6K render and
the response's `model` field shows the base slug, so the billing always
