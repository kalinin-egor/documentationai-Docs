---
title: LLM & Image API
description: Chat, reasoning, and image generation through a single OpenAI-compatible API. Claude, GPT-5, Gemini, Grok, Nano Banana, and more — one key, one balance.
---

## Overview

GPUniq provides a single API surface for 80+ language models and image generators across Anthropic, OpenAI, Google, and xAI. One API key, one balance, one usage dashboard — chat completions, reasoning, and text-to-image all in the same place.
...
<Info>
  ...
  short jobs, but set your client timeout to at least 120 seconds if
  you use it for the slower models.
</Info>
...
returns — a timed-out or failed job costs nothing.
