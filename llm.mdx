...
| Kling O3 (Video) | `kling-o3-video` | from $0.076 / s | 720p / 1080p / 4K, native audio, 3–15s; t2v / i2v, first+last frame, reference images (≤4) + reference video |
...
- **Reference-to-video** — up to **4 reference images** via
  `reference_image_urls`, which define the subject/style.
- **Video reference (video-to-video)** — `video_url` / `video_urls`
  supply the motion and camera language. O3 consumes a reference clip
  **only in reference-to-video mode**, so at least one
  `reference_image_urls` entry must accompany it; a clip on its own is
  rejected with a 400. `image_url` / `last_frame_url` may still ride
  along as start / end anchors. For pure video-to-video with no
  reference image, use `kling-2-6-motion-control` /
