...
GPUniq surfaces three escape hatches — pick the one that fits your
client. **Either of them** keeps the connection alive past the 100 s
...
If you call the OpenAI-compat endpoint with `max_tokens > 4096` and
`stream` unset, GPUniq attaches an `X-GPUniq-Warning` response header
nudging you to switch to streaming.

### 2. Job-based long-poll API
...
### 3. Synchronous request without streaming
